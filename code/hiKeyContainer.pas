...
begin
  ...
  if Name <> '' then
  begin
    if not CryptAcquireContext(@hProv, @Name[1], MS_ENHANCED_PROV, PROV_RSA_FULL, 0) then
    begin
      if CryptAcquireContext(@hProv, @Name[1], MS_ENHANCED_PROV, PROV_RSA_FULL, CRYPT_NEWKEYSET) then
...
begin
  ...
  if Name <> '' then
  begin
    if CryptAcquireContext(@hProv, @Name[1], MS_ENHANCED_PROV, PROV_RSA_FULL, 0) then
    begin
      if CryptAcquireContext(@hProv, @Name[1], MS_ENHANCED_PROV, PROV_RSA_FULL, CRYPT_DELETEKEYSET) then
...
begin
  ...
  begin
    ...
    if CryptAcquireContext(@hProv, @Name[1], MS_ENHANCED_PROV, PROV_RSA_FULL, CRYPT_SILENT) then
    begin
